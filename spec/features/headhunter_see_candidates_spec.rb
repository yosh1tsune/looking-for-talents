...
feature 'headhunter see candidates' do
    ...
    scenario 'successfully from opportunity#show when candidates are registered' do
        ...
        another_registration = CandidateRegistration.create!(opportunity: opportunity, candidate: another_candidate, registration_resume: '3 anos de experiencia como analista de suporte')
        third_registration = CandidateRegistration.create!(opportunity: opportunity, candidate: third_candidate, registration_resume: 'Projetos acadêmicos')
    ...
    scenario 'and visualize their profiles' do
        ...
        opportunity = Opportunity.create!(title: 'Desenvolvedor Júnior Ruby on Rails', company: 'RR Systems', work_description: 'Desenvolvimento de aplicações web', 
                            ...
                            CandidateRegistration.create!(opportunity: opportunity, candidate: candidate, registration_resume: '1 ano de estágio com desenvolvimento web')
        CandidateRegistration.create!(opportunity: opportunity, candidate: another_candidate, registration_resume: '3 anos como analista de suporte')
    ...
    xscenario 'and highlight them' do
        ...
        expect(page).to have_content(profile.name)
        expect(page).to have_content("#{I18n.t('birth_date')}: #{I18n.l(profile.birth_date)}")
        expect(page).to have_content("#{I18n.t('document')}: #{profile.document}")
        expect(page).to have_content("#{I18n.t('scholarity')}: #{profile.scholarity}")
        expect(page).to have_content("#{I18n.t('professional_resume')}: #{profile.professional_resume}")
        expect(page).to have_content("#{I18n.t('address')}: #{profile.address}")
        expect(current_path).to eq profile_path(candidate)
        expect(page).to have_link 'Remover destaque'
    end

    xscenario 'and remove hightlights' do
        headhunter = Headhunter.create!(email: 'headhunter@email.com', password: 'head1234')
        candidate = Candidate.create!(email: 'candidate@email.com', password: 'cand1234')
        profile = Profile.create!(name: 'Bruno Silva', birth_date: '22/04/1996', document: '996.490.558-00', scholarity: 'Superior Incompleto', 
                                professional_resume: 'Desenvolvimento web com Dart 2', address: 'Alameda Santos, 1293', candidate: candidate, highlighted: true)
